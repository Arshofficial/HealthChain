...
export function PendingAccessControl() {
        ...
        const { error: insertError } = await supabase.from("provider_patient").insert([
          {
            ...
            valid_till: new Date(new Date().setDate(new Date().getDate() + 30)), // 30 days validity
          },
        ]);
  ...
  if (loading) return <p className="text-muted-foreground">Loading pending requests...</p>;
